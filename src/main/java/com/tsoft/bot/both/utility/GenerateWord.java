...
public class GenerateWord {
    ...
    private static String TEMP_SEQUENCE;
    private static String TEMP_SCENERY;
    private static XWPFDocument document;
    private static XWPFRun run;
    private static FileOutputStream fileOutputStream;
    private static String NAME_ALTER = null;

    public void configureAlterName(String name) {
        if (name.contains("RENAMED BY GHERKIN TEST")){
            name = name + " - " + TEMP_SCENERY;
            name = name.replaceAll("RENAMED BY GHERKIN TEST","");
        } else if (name.contains("RENAMED BY EXCEL")) {
            name = name.replaceAll("RENAMED BY EXCEL", "");
        }
        NAME_ALTER = name;
    }
    ...
    public static void startUpWord(String name) throws IOException {
        ...
        try {
            ...
            TEMP_SEQUENCE = generarSecuencia();
            TEMP_WORD_FILE = FileUtils.getFile(carpeta) + "/" + name + "-" + TEMP_SEQUENCE + WORD_EXTENSION;
            fileOutputStream = new FileOutputStream(FileUtils.getFile(carpeta) + "/" + name + "-" + TEMP_SEQUENCE + WORD_EXTENSION);
    ...
    public void sendText(String texto) {
        run.setText("Fecha : " + generarFecha() + ", Hora : " + generarHora() + " |");
        run.addBreak();
        int line = 0;
        String[] text = texto.split("\n");
        do {
            line += 1;
            run.setText(text[line - 1]);
            if (line != text.length)
                run.addBreak();
    ...
    public void endToWord(String status) throws IOException {
        try {
            ...
            String pathName;
            if (NAME_ALTER != null) {
                pathName = TEMP_WORD_FILE.split("\\\\target\\\\")[0] + "\\target\\resultado\\" + NAME_ALTER + TEMP_SEQUENCE + status.toUpperCase() + WORD_EXTENSION ;
            } else {
                pathName = TEMP_WORD_FILE.split("\\.docx")[0] + "-" + status.toUpperCase() + WORD_EXTENSION;
            }

            File fileWithNewName = new File(pathName);
